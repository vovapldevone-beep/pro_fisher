...
- **`resources/js/locales/uk.json`** та **`pl.json`** — переклади. Секції: `nav`, `header`, `modal`, `catch`, `post`, `posts`, `map`, `auth`, `common`, `cabinet`, `stats`, `time`, `activity`, `achievements`, `fisher`, `friends`, `fish`, `raffle`. Сторінки Login/Register теж перекладені — усе в секції `auth` (заголовки, підписи полів, кнопки Google, помилки OAuth `errorGoogle`/`errorBlocked`); раніше вони містили польський текст прямо в шаблоні. `i18n.js` виставляє `document.documentElement.lang` не лише в `setLocale`, а й одразу при завантаженні збереженої мови — інакше польська сторінка приходила з `lang="uk"` із блейд-шаблону.
...
**Composables** (all in `composables/`): `useLike(sourceGetter, onChange)` holds one publication's like state for both `PostCard` and `CatchDetailModal` (optimistic toggle, revert on failure, guests pushed to `/login`). It watches **`is_liked`/`likes_count`, not the object** — the pages update the publication in place, which leaves the reference untouched, so watching the getter alone never fired and the card under the modal kept showing the old heart. `useScrollLock(isLocked)` freezes `<main>` (the app scrolls there, not `<body>`). `useHideOnScroll(mainRef, {height})` returns `offset` (px the header is pushed up), `hidden`, `settling` and `show()`. It tracks the scroll **continuously** — the header moves exactly as far as you scrolled, in both directions, and counts as hidden once its bottom edge meets the top of the screen. It used to be a boolean flipped past an 8 px threshold, so one flick sent the header the whole way up in a 300 ms transition, which reads as a flinch on a phone. Near the top the scroll position acts as a **cap** on the offset, not as its value: it guarantees a whole header at `scrollTop` 0 with no drift, but assigning it pushed an already-revealed header back down — a flick to the top revealed it at y=300, then y=60 shoved it 60 px up and it slid in a second time. `height` may be a ref, and App.vue passes the **measured** header height rather than the 65 px the layout reserves: the mobile header is 70 px, and the missing 5 px stayed visible at the top edge. After ~140 ms of stillness a half-open header snaps to the nearer end (`settling` turns the CSS transition on for that alone — a transition while tracking would lag the finger). Still ignores the iOS rubber-band overscroll region; the old bottom dead-zone is gone, since hiding no longer changes any height. `useInfiniteScroll(sentinelRef, {loading, hasMore, onLoad})` re-attaches its `IntersectionObserver` when the sentinel remounts (tab switches) and keeps firing while the sentinel stays visible; `root` is `<main>`. Note the pagination invariant: `page` holds the *next* page, so `hasMore` uses `page <= lastPage` (inclusive) or the final page never loads. `useGeolocation()` wraps `navigator.geolocation` + Nominatim reverse-geocode (HTTPS-only guard).

**App layout (`App.vue`)**: root div is `flex flex-col overscroll-none` with `height: 100dvh` and `overflow-hidden`; `<main>` (ref `mainEl`) has `min-h-0 flex-1 overflow-y-auto overscroll-y-none` — content scrolls inside main, not the body; `overscroll-*none` kills the iOS rubber-band (which otherwise made the header flap). The header's 65 px of space is `padding-top` **inside `<main>`**, not on the content row: sliding the header away then changes no element's height, so `scrollHeight` stays put and the content never jumps. That is what makes continuous tracking possible — the old collapsing row padding shifted the content by 65 px every time the flag flipped. Mobile bottom nav is a **static flex child** (`shrink-0 md:hidden`) at the bottom of the column — no `position: fixed`. A raised circular **"+" FAB** is injected right after the "Карта" tab (rendered inside the nav's `v-for` via `item.to === '/map'`): styled like the other nav items — `oklch(0.24 0.03 269.9)` background (inline style), grey `text-white/60` icon — with a thin `border-2 border-[#1a1f2e]` (menu colour), sitting flush within the bar. Tapping it calls `openAddPublication()` → `router.push('/cabinet?action=add-post')`, opening the tabbed `AddPostModal`. `AppSidebar.vue` is desktop-only (`hidden md:flex`, fixed left, expands on hover). Full-height pages (MapPage) use `h-full`.

**Header** (`AppHeader.vue`): brand is `<img :src="'/images/logo.png'">` (`:src`, not `src`, so Vite serves it from `public/` instead of bundling) + name **"ProFisher"**. Slides up and down with the scroll on mobile via `useHideOnScroll`, driven by a `--header-offset` CSS variable so the "never hides from `md` up" rule can stay a media query instead of a JS breakpoint listener. Language switcher (dropdown, see Internationalisation) always visible. Shows the fish-hunt counter (see Fish hunt). App.vue renders a floating fish badge (top-right, mobile, Posts page only) when the header is hidden.
